...
void cargarMapa(char mapa[ALTO][ANCHO], float *posX, float *posY);
...
int main() {
    ...
    al_key_down(&teclado, ALLEGRO_KEY_W);
    al_key_down(&teclado, ALLEGRO_KEY_S);
    al_key_down(&teclado, ALLEGRO_KEY_A);
    al_key_down(&teclado, ALLEGRO_KEY_D);
    al_key_down(&teclado, ALLEGRO_KEY_ESCAPE);
   ...
   while (salir) {
    ...
    if (evento.type == ALLEGRO_EVENT_TIMER) {
        ...
        colision(mapa, &posX, &posY, nuevaX, nuevaY);
...
void cargarMapa(char mapa[ALTO][ANCHO], float *posX, float *posY) {
    ...
    if (!archivofop) {
        ...
        for (int i = 0; i < ALTO; i++)
            for (int j = 0; j < ANCHO; j++)
                mapa[i][j] = '.';
        *posX = TAMANO_CUADRADO;
        *posY = TAMANO_CUADRADO;
        return;
    }
    ...
    for (int i = 0; i < ALTO; i++) {
        for (int j = 0; j < ANCHO; j++) {
            ...
            if (mapa[i][j] == '@') { // jugador en el mapa
                *posX = j * TAMANO_CUADRADO;
                *posY = i * TAMANO_CUADRADO;
            }
        }
    }
    fclose(archivofop);
